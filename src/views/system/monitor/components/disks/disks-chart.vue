...
<script>
  ...
  export default {
    ...
    computed: {
      chartData() {
        ...
        if (this.chartOptionData.length>0) {
          var dataObj = {
            seriesData: this.chartOptionData,
            category: []
          };
    ...
    methods: {
        ...
        if (optionData.seriesData) {
          this.chart.setOption({
            toolbox: {
              ...
              feature: {
                myTool1: {
                  show: optionData.seriesData.length > 4 ? true : false,
                  title: '更多',
                  icon: 'image://'+ require('assets/img/echarts/more.png'),
                  onclick: function (){
                    const newRoutes = [
                      {
                        name: 'muti-disks-charts',
                        path: '/muti-charts',
                        component: () => import('@/views/system/monitor/components/disks/muti-disks-charts.vue')
                      }
                    ]
                    that.$router.addRoutes(newRoutes)
                    that.$router.push({name: 'muti-disks-charts'})
                  }
                }
              }
            },
      ...
      setSeriesData (val) {
        ...
        seriesData.map((item, index) => {
          ...
          var seriesObj = {
            ...
            center: this.setCenter(seriesData.length, index),//图表相对于盒子的位置 第一个是水平的位置 第二个是垂直的值 默认是[50%,50%]是在水平和垂直方向居中 可以设置百分比 也可以设置具体值
      ...
      setCenter(length, index) {
        if(length == 1) {
          return ['50%','50%']
        } else if(length == 2) {
          return [33*(index+1) + '%','50%']
        } else if(length === 3) {
          return [30*(index+1)-10 +'%','50%']
        } else {
          return [25 * (index + 1) - 13.5 + '%', '50%']
        }
      },
      // 自动播放tooltip
      autoPlay() {
