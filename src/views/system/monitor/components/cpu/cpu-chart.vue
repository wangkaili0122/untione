  ...
  export default {
    ...
    data() {
      return {
        chart: null,
        // chartData: {
        //   optionData: JSON.parse(JSON.stringify(this.chartOptionData))
        // },
        time: null
      };
    },
    computed: {
      chartData() {
        let Obj = {}
        if (this.chartOptionData) {
          // let total =  parseFloat(((this.chartOptionData.sys + this.chartOptionData.used)/100).toFixed(4))
          let total =  (this.chartOptionData.sys*100 + this.chartOptionData.used*100)/10000
          var dataObj = {
            seriesData: [{
              name: 'CPU利用率',
              value: total
