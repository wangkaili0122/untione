...
<script>
  ...
  import dictGet from 'mixins/dict-get'
  //引入基本模板
  ...
  import { GetAppAccess } from 'request/services/app-statistics'
  // const colorList = ['#41a6f7', '#ffc500', '#26c7db', '#7dc57e', '#FEC400', '#F55D90', '#ca8622', '#bda29a', '#6e7074', '#546570', '#c4ccd3'];
  ...
  export default {
    ...
    methods: {
      getOptionData() {
        GetAppAccess(this.typeId).then(res => {
          ...
          if (resData && resData.length > 0) {
            ...
            resData.map((item, index) => {
              item.name = this.getDict(item.name, 'sys_app_access')
            });
            var dataObj = {
              seriesData: resData,
              category: []
            ...
            this.rawHtml = '<div id="chart3-nodata" style="text-align: center;font-weight: bold;line-height: 140px; color: #666">暂无数据</div>';
          }
        }).catch(error => {
          this.rawHtml = '<div id="chart3-nodata" style="text-align: center;font-weight: bold;line-height: 140px; color: #666">暂无数据</div>';
