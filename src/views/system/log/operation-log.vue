  ...
  <div>
    <transition name="el-zoom-in-top">
      <el-form v-show="searchShow" v-model="queryCondition" :inline="true" size="mini">
        <el-form-item label="登录名" class="filter-item-margin">
          <el-input clearable v-model="queryCondition.operName"
    ...
    <el-row :gutter="10" class="optBar">
      <el-button icon="el-icon-delete" plain type="primary" size="mini" @click="batchDelete"
                 v-hasPerm="'LOG_OPER_BATCHDEL_B'"
                 :disabled="selectionData.length === 0">批量删除</el-button>
      <el-button icon="el-icon-download" plain type="primary" size="mini" @click="operationExport">批量导出</el-button>
      ...
      <el-table
        ...
        <el-table-column
          ...
          <template slot-scope="scope">
            <span>{{(scope.row.operName && scope.row.operName.length>0)?scope.row.operName:'游客'}}</span>
          </template>
        </el-table-column>
        <!--<el-table-column-->
        <!--prop="deptName"-->
        ...
        <el-table-column
          ...
          min-width="80">
          <template slot-scope="scope">
            {{scope.row.businessType|dictFilter('sys_operation_log_type')}}
          </template>
        </el-table-column>
        <el-table-column
          prop="status"
          ...
          <template slot-scope="scope">
            <el-tag :type="scope.row.status == '0'? 'success' : 'info'" size="mini">
              {{scope.row.status|dictFilter('sys_log_status')}}
            </el-tag>
          </template>
        </el-table-column>
        <el-table-column
          ...
          <template slot-scope="scope">
            <div>
              <el-button
                ...
                plain
                type="danger"
                v-hasPerm="'LOG_OPER_DELETE_B'"
                @click="deleteHandle(scope.row.operId)">
                删除
              </el-button>
            </div>
          </template>
...
<script>
  ...
  import { GetOperateLogs, DeleteOperateLog, DeleteOperateLogBatch, DownloadOperateList } from 'request/services/log.js'
  ...
  export default {
      ...
      return {
        ...
        pickerOptions: {
          ...
          disabledDate: (time) => {
            ...
            //最近7天就是算上今天是7天，所以应该减去六天的时间，上面同理
            const Day = 6
            let secondNum = Day * 24 * 60 * 60 * 1000
            if (timeOptionRange) {
    ...
    methods: {
      ...
      getTableData: function () {
        ...
        GetOperateLogs(condition).then(res => {
      ...
      batchDelete: function () {
        this.$confirm('请确认是否删除当前选中的日志？', '提示', {
          ...
          DeleteOperateLogBatch(this.selectionDataIds).then(res => {
            this.$notify({
              title: '成功',
              message: '批量删除成功',
              type: 'success'
      ...
      operationExport () {
        ...
        DownloadOperateList(data).then(res => {
          exportFile(res, 'xls', '操作日志导出列表')
        })
      }
    },
    mounted: function () {
